...
import de.sambi.arroundme.dto.DepartementDTO;
import de.sambi.arroundme.editors.DepartmentEditor;
import de.sambi.arroundme.model.DepartmentEntity;
import de.sambi.arroundme.model.EmployeeEntity;
import de.sambi.arroundme.service.EmployeeManager;
...
public class EmployeeController
{
	...
	@RequestMapping(value = "/allEmployee", method = RequestMethod.GET, produces = "application/json")
	public @ResponseBody List<DepartementDTO>  testRestController(){
		System.out.println("manager.getAllDepartments() "+manager.getAllDepartments().size());
		List<DepartmentEntity>  vRp = manager.getAllDepartments();
		
		List<DepartementDTO> vRp1  = new ArrayList<DepartementDTO>();
		for(DepartmentEntity vItem : vRp){
			DepartementDTO 		 vItemDTO = new DepartementDTO();
			vItemDTO.setName(vItem.getName());
			vRp1.add(vItemDTO);
		}
